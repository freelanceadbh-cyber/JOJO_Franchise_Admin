...
function updateFile(filepath, imgPrefix) {
    let content = fs.readFileSync(filepath, 'utf-8');
    const startTag = '<!-- Add milkshake items here -->';
    if (content.includes(startTag)) {
        // Replace all {img_prefix}
        let replacement = html_block.split('{img_prefix}').join(imgPrefix);
        content = content.replace(startTag, replacement);
        fs.writeFileSync(filepath, content, 'utf-8');
        console.log(`Updated ${filepath}`);
    } else {
        console.log(`Could not find start tag in ${filepath}`);
    }
}

updateFile('d:\\Projects\\JoJo Ice creams\\index.html', 'images/flavors/');
updateFile('d:\\Projects\\JoJo Ice creams\\Ice-cream-Shop\\index.html', 'assets/images/flavors/');
